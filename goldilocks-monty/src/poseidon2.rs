//! Implementation of Poseidon2 for Goldilocks-Monty field
//! Adapted from the Goldilocks implementation to work with Montgomery form
...
/// Degree of the chosen permutation polynomial for Goldilocks-Monty, used as the Poseidon2 S-Box.
///
/// As p - 1 = 2^32 * 3 * 5 * 17 * ... the smallest choice for a degree D satisfying gcd(p - 1, D) = 1 is 7.
const GOLDILOCKS_S_BOX_DEGREE: u64 = 7;

/// An implementation of the Poseidon2 hash function for the Goldilocks-Monty field.
///
...
/// A recreating of the Poseidon2 implementation by Horizen Labs for the Goldilocks-Monty field.
///
/// It acts on arrays of the form `[Goldilocks; WIDTH]`
...
// Matrix diagonal constants converted to Montgomery form
pub const MATRIX_DIAG_8_GOLDILOCKS_MONTY: [Goldilocks; 8] = Goldilocks::new_array([
    0xa98811a1fed4e3a5,
    0x1cc48b54f377e2a0,
    0xe40cd4f6c5609a26,
    0x11de79ebca97a4a3,
...
pub const MATRIX_DIAG_12_GOLDILOCKS_MONTY: [Goldilocks; 12] = Goldilocks::new_array([
    0xc3b6c08e23ba9300,
    0xd84b5de94a324fb6,
    0x0d0c371c5b35b84f,
    0x7964f570e7188037,
...
pub const MATRIX_DIAG_16_GOLDILOCKS_MONTY: [Goldilocks; 16] = Goldilocks::new_array([
    0xde9b91a467d6afc0,
    0xc5f16b9c76a9be17,
    0x0ab0fef2d540ac55,
    0x3001d27009d05773,
...
pub const MATRIX_DIAG_20_GOLDILOCKS_MONTY: [Goldilocks; 20] = Goldilocks::new_array([
    0x95c381fda3b1fa57,
    0xf36fe9eb1288f42c,
    0x89f5dcdfef277944,
    0x106f22eadeb3e2d2,
...
/// The internal layers of the Poseidon2 permutation for Goldilocks-Monty.
    ...
    InternalLayer<A, 8, GOLDILOCKS_S_BOX_DEGREE> for Poseidon2InternalLayerGoldilocks
{
    ...
    fn permute_state(&self, state: &mut [A; 8]) {
        internal_permute_state(
            state,
            |x| matmul_internal(x, MATRIX_DIAG_8_GOLDILOCKS_MONTY),
            &self.internal_constants,
        )
    }
}
    ...
    InternalLayer<A, 12, GOLDILOCKS_S_BOX_DEGREE> for Poseidon2InternalLayerGoldilocks
{
    ...
    fn permute_state(&self, state: &mut [A; 12]) {
        internal_permute_state(
            state,
            |x| matmul_internal(x, MATRIX_DIAG_12_GOLDILOCKS_MONTY),
            &self.internal_constants,
        )
    }
}
    ...
    InternalLayer<A, 16, GOLDILOCKS_S_BOX_DEGREE> for Poseidon2InternalLayerGoldilocks
{
    ...
    fn permute_state(&self, state: &mut [A; 16]) {
        internal_permute_state(
            state,
            |x| matmul_internal(x, MATRIX_DIAG_16_GOLDILOCKS_MONTY),
            &self.internal_constants,
        )
    }
}
    ...
    InternalLayer<A, 20, GOLDILOCKS_S_BOX_DEGREE> for Poseidon2InternalLayerGoldilocks
{
    ...
    fn permute_state(&self, state: &mut [A; 20]) {
        internal_permute_state(
            state,
            |x| matmul_internal(x, MATRIX_DIAG_20_GOLDILOCKS_MONTY),
            &self.internal_constants,
        )
    }
}

/// The external layers of the Poseidon2 permutation for Goldilocks-Monty.
#[derive(Clone)]
...
/// The external layers of the Poseidon2 permutation used by Horizen Labs for Goldilocks-Monty.
#[derive(Clone)]
...
// External round constants converted to Montgomery form
pub const HL_GOLDILOCKS_MONTY_8_EXTERNAL_ROUND_CONSTANTS: [[[u64; 8]; 4]; 2] = [
    [
        [
            0xdd5743e7f2a5a5d9,
            0xcb3a864e58ada44b,
...
pub const HL_GOLDILOCKS_MONTY_8_INTERNAL_ROUND_CONSTANTS: [u64; 22] = [
    0x488897d85ff51f56,
    0x1140737ccb162218,
    0xa7eeb9215866ed35,
    0x9bd2976fee49fcc9,
...
mod tests {
    ...
    // A function which recreates the poseidon2 implementation for Goldilocks-Monty
    fn hl_poseidon2_goldilocks_monty_width_8(input: &mut [F; 8]) {
        const WIDTH: usize = 8;
