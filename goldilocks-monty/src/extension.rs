...
impl BinomiallyExtendable<2> for Goldilocks {
    ...
    const W: Self = Goldilocks::new(7);

    // DTH_ROOT = W^((p - 1)/2).
    const DTH_ROOT: Self = Goldilocks::new(18446744069414584320);
...
impl HasTwoAdicBinomialExtension<2> for Goldilocks {
    ...
    fn ext_two_adic_generator(bits: usize) -> [Self; 2] {
        ...
        if bits == 33 {
            [Self::ZERO, Goldilocks::new(15659105665374529263)]
        } else {
...
impl BinomiallyExtendable<5> for Goldilocks {
    ...
    //  ```
    const W: Self = Goldilocks::new(3);

    // 5-th root = w^((p - 1)/5)
    const DTH_ROOT: Self = Goldilocks::new(1041288259238279555);
